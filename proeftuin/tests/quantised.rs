...
macro_rules! mappings {
    ($gpu:expr, $kind:expr, $seed:expr, $half:literal, $whole:literal, $double:literal) => {
        vec![
            check::<$half>($gpu, $kind, "clustered", $seed),
            check::<$whole>($gpu, $kind, "whole", $seed),
        ...
        for seed in 0..6_u64 {
            ...
            for outcome in outcomes {
                match outcome {
                    ...
                    // a skipped check that looks green is worse than a red one.
                    Outcome::Refused(why) => {
                        eprintln!("  {} not run at seed {seed}: refused — {why}", kind.name());
                    }
                    Outcome::Unsupported(missing) => {
                        eprintln!(
                            "  {} not run at seed {seed}: device lacks {missing:?}",
                            kind.name()
                        );
                    }
                    ...
                    Outcome::Invalid(complaint) => complaints.push(format!(
                        "{} seed {seed}: spirv-val rejected it — {complaint}",
                        kind.name()
                    )),
