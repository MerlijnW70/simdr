...
use proeftuin::halves::{EDGES, PATTERNS, roundtrip};
use runner::Gpu;
...
fn every_half_pattern_survives_a_load_and_a_store() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (sent, lost) = match outcome {
        Answer::Ran(round) => (round.sent, round.lost),
        // Lost coverage rather than failure: `f16` needs `Float16` and 16-bit storage, and a device
        // without them is being honest. Refusals and unsupported are the same class here.
        ...
        Answer::Invalid(complaint) => {
            panic!("spirv-val rejected the identity kernel: {complaint}")
        }
        Answer::Errored(error) => {
    ...
    assert_eq!(
        sent, PATTERNS,
        "the sweep must be exhaustive to mean anything"
    );
    ...
    for (name, bits) in EDGES {
        ...
        println!(
            ...
            changed.map_or_else(
                ...
                |l| format!("became {:#06x}", l.returned)
            )
        );
    }
    ...
    if !nans.is_empty() {
        println!(
            "  {} NaN patterns came back reshaped, which Vulkan permits",
            nans.len()
        );
    }
