...
**If you are here to use this rather than to read it, `USING.md` is the shorter document.** It is
the promises this crate makes, what enforces each one, what it asks of you in return, and what is
deliberately missing — written for somebody outside this repository, which is a reader it has never
had. Its examples compile: `src/lib.rs` pulls it in under `cfg(doctest)`, so a guide that stops
working stops the build.
...
- `decisions/` — the <!--count:decisions-->10 decisions that shape everything, and why. `DR-0002` carries a correction
